...
├── saved/                  # Сохранённые конфигов, логов и чекпоинтов экспериментов
├── weights/                # Предобученные веса моделей
├── img/                    # Изображения в README.md
└── src/
    ...
    ├── pipelines/         # Пайплайны eda и аннотации
    ├── trainer/           # Тренер, инференсер
    ├── transforms/        # Аугментации и преобразования
...
Я не была уверена, что можно делать датасет публичным, поэтому вместо Roboflow использовала LabelStudio, выгрузила в формате coco и дальше сама разбила на train/test/val в соотношении 80/10/10.
...
* для обучения и инференса
   ...
   ```
4. Для логирования авторизироваться в Weights & Biases (нужен VPN):
   ```bash
   wandb login
   ```
2. Для аннотирования скачать веса моделей и положить их в папку `weights/`:
   ```bash
   mkdir -p weights
...
Сначала я запускала со следующими параметрами: 25 эпох, AdamW с небольшим lr=5e-5 при батче 2 и без аугментаций, но модель особо не обучилась.

Во втором эксперименте оптимайзер заменила на SGD, для избегания резких скачков lr взяла lr_scheduler CosineAnnealingLR, начальный lr=0.001. Чтобы модель успела сойтись на нашем маленьком датасете, увеличила количество эпох до 80, добавила early stop после 10 эпох без улучшений и аугментации: поворот, отражение и др (полный список гиперпараметров можно увидеть в конфигах).
...
Метрика - COCO mAP@[IoU=0.50:0.95] из torchmetrics, потому что она отражает как точность обнаружения линейки, так и качество её локализации.

![alt text](<img/image copy 3.png>)

Лосс на валидационной выборке уменьшается, на трейне скачет, возможно, из-за аугментаций, но метрика повышается, так что обучается хорошо.
...
Для сравнения можно ориентироваться на [0.46](https://docs.pytorch.org/vision/main/models/generated/torchvision.models.detection.fasterrcnn_resnet50_fpn_v2.html) и в районе 0.7 для моделей из [ultralitics](https://blog.roboflow.com/best-object-detection-models/).

### Следующие шаги:
* конечно, лучше попробовать более сильные модели (например, можно взять что-то по [бенчмаркам](https://blog.roboflow.com/best-object-detection-models/)), просто по заданию нужно было использовать pytorch, а YOLO и другие ultralitics модели - это обращение к black-box, хотя при желании можно залезть в их код и поэкспериментировать с архитектурой
* расширение датасета
...
1. Мне хотелось сделать хорошую структуру кода, но в итоге я на нее потратила много времени, так как раньше не работала с таким шаблоном, в итоге на эксперименты ничего не осталось (во всяком случае до того дедлайна, сроки которого я указала). Тут всегда баланс между поддерживаемостью и быстротой разработки.
